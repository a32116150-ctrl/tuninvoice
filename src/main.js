...
setInterval(() => {
    ...
    try {
        ...
        due.forEach(r => {
            if (Notification.isSupported()) new Notification({ title: '⏰ Rappel TuniInvoice', body: r.title }).show();
...
// ==================== EXPENSES ====================
ipcMain.handle('expenses:getAll',  async (_, { userId, filters }) => { try { return db.getExpenses(userId, filters||{}); } catch { return []; } });
...
ipcMain.handle('expenses:delete',  async (_, id) => { try { db.deleteExpense(id); return { success: true }; } catch (e) { return { success: false, error: e.message }; } });
...
ipcMain.handle('tools:openCalculator', async () => {
    try {
        ...
        calculatorWindow = new BrowserWindow({
            width: 520, height: 680, title: 'Calculatrice Fiscale — TuniInvoice Pro',
