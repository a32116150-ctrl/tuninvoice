...
class AppDatabase {
    ...
    initTables() {
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS user_settings (user_id TEXT PRIMARY KEY, prefix_facture TEXT DEFAULT 'FAC', prefix_devis TEXT DEFAULT 'DEV', prefix_bon TEXT DEFAULT 'BC', prefix_retenue TEXT DEFAULT 'RS', prefix_avoir TEXT DEFAULT 'AV', prefix_contract TEXT DEFAULT 'CTR', decimal_places INTEGER DEFAULT 3, rounding_method TEXT DEFAULT 'half_up', document_theme TEXT DEFAULT NULL, currency_default TEXT DEFAULT 'TND', FOREIGN KEY (user_id) REFERENCES users(id))`);
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS expenses (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, date TEXT NOT NULL, category TEXT NOT NULL, description TEXT, amount REAL NOT NULL, tva REAL DEFAULT 0, supplier TEXT, reference TEXT, receipt_image TEXT, payment_method TEXT DEFAULT 'virement', created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
    ...
    loginUser(email, password) {
        ...
        if (!user || !bcrypt.compareSync(password, user.password_hash)) throw new Error('Identifiants invalides');
    ...
    getOverdueDocuments(userId) {
        const today = new Date().toISOString().split('T')[0];
        return this.db.prepare(`SELECT * FROM documents WHERE user_id=? AND type='facture' AND payment_status != 'paid' AND due_date IS NOT NULL AND due_date < ? ORDER BY due_date ASC`).all(userId, today).map(d => this.formatDocument(d));
    }
    formatDocument(doc) {
        return {
    ...
    getDashboardStats(userId) {
        ...
        const totalExpenses = this.db.prepare(`SELECT COALESCE(SUM(amount),0) as total FROM expenses WHERE user_id=? AND strftime('%Y',date)=strftime('%Y','now')`).get(userId).total;
        ...
        return { totalDocs, totalRevenue, totalClients, thisMonth, unpaidCount, unpaidTotal, totalExpenses, monthlyRevenue, typeBreakdown, topClients, recentActivity, overdueCount, totalRetenues };
    }
    ...
    saveExpense(data) {
        ...
        if (ex) {
            this.db.prepare(`UPDATE expenses SET date=?,category=?,description=?,amount=?,tva=?,supplier=?,reference=?,receipt_image=?,payment_method=? WHERE id=?`).run(data.date, data.category, data.description||null, data.amount, data.tva||0, data.supplier||null, data.reference||null, data.receiptImage||null, data.paymentMethod||'virement', id);
        } else {
            this.db.prepare(`INSERT INTO expenses (id,user_id,date,category,description,amount,tva,supplier,reference,receipt_image,payment_method) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(id, data.userId, data.date, data.category, data.description||null, data.amount, data.tva||0, data.supplier||null, data.reference||null, data.receiptImage||null, data.paymentMethod||'virement');
        }
    ...
    getExpenses(userId, filters = {}) {
        ...
        const params = [userId];
        if (filters.year) { q += ` AND strftime('%Y',date)=?`; params.push(String(filters.year)); }
        if (filters.month) { q += ` AND strftime('%m',date)=?`; params.push(String(filters.month).padStart(2, '0')); }
        if (filters.category) { q += ` AND category=?`; params.push(filters.category); }
        q += ' ORDER BY date DESC';
        return this.db.prepare(q).all(...params);
    }
    deleteExpense(id) { this.db.prepare('DELETE FROM expenses WHERE id=?').run(id); }
    getExpenseSummary(userId, year) {
        const y = year || new Date().getFullYear();
        const byCategory = this.db.prepare(`SELECT category, COALESCE(SUM(amount),0) as total, COUNT(*) as count FROM expenses WHERE user_id=? AND strftime('%Y',date)=? GROUP BY category ORDER BY total DESC`).all(userId, String(y));
        const monthly = this.db.prepare(`SELECT strftime('%m',date) as month, COALESCE(SUM(amount),0) as total FROM expenses WHERE user_id=? AND strftime('%Y',date)=? GROUP BY month ORDER BY month ASC`).all(userId, String(y));
        const totalYear = this.db.prepare(`SELECT COALESCE(SUM(amount),0) as total FROM expenses WHERE user_id=? AND strftime('%Y',date)=?`).get(userId, String(y)).total;
