...
class AppDatabase {
    ...
    runMigrations() {
        ...
        try {
            const svcCols = this.db.prepare('PRAGMA table_info(services)').all().map(c => c.name);
        ...
        } catch (e) { console.error('[migration] services columns:', e.message); }
        ...
        try {
            const docCols = this.db.prepare('PRAGMA table_info(documents)').all().map(c => c.name);
        ...
        } catch (e) { console.error('[migration] documents columns:', e.message); }
    ...
    initTables() {
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS services (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, name TEXT NOT NULL, description TEXT, price REAL DEFAULT 0, tva REAL DEFAULT 19, category TEXT, unit TEXT DEFAULT 'unité', barcode TEXT, stock INTEGER DEFAULT 0, min_stock INTEGER DEFAULT 0, created_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS relances (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, invoice_id TEXT NOT NULL, attempt INTEGER DEFAULT 1, method TEXT DEFAULT 'pdf', recipient_email TEXT, sent_at DATETIME DEFAULT CURRENT_TIMESTAMP, notes TEXT)`);
        ...
        this.db.exec(`CREATE TABLE IF NOT EXISTS exchange_rates (id TEXT PRIMARY KEY, user_id TEXT NOT NULL, currency TEXT NOT NULL, rate REAL DEFAULT 1.0, updated_at DATETIME DEFAULT CURRENT_TIMESTAMP)`);
    ...
    restore(backupPath) {
        ...
        this.db.close(); fs.copyFileSync(backupPath, this.dbPath); this.db = new Database(this.dbPath); this.db.pragma('journal_mode = WAL'); this.db.pragma('foreign_keys = ON'); this.initTables();
    }
    ...
    getDocumentsPaginated(userId, page, pageSize) {
        ...
        const rows = this.db.prepare('SELECT * FROM documents WHERE user_id=? ORDER BY created_at DESC LIMIT ? OFFSET ?').all(userId, pageSize, offset).map(d => this.formatDocument(d));
    ...
    saveService(d) {
        ...
        if (ex) {
            this.db.prepare(`UPDATE services SET name=?,description=?,price=?,tva=?,category=?,unit=?,barcode=?,stock=?,min_stock=? WHERE id=?`).run(d.name, d.description||null, d.price, d.tva, d.category||null, d.unit||'unité', d.barcode||null, d.stock||0, d.minStock||0, id);
        } else {
            this.db.prepare(`INSERT INTO services (id,user_id,name,description,price,tva,category,unit,barcode,stock,min_stock) VALUES (?,?,?,?,?,?,?,?,?,?,?)`).run(id, d.userId, d.name, d.description||null, d.price, d.tva, d.category||null, d.unit||'unité', d.barcode||null, d.stock||0, d.minStock||0);
        }
        return { id, ...d };
    }
    ...
    saveRelance(data) {
        const id = data.id || uuidv4();
        this.db.prepare(`INSERT INTO relances (id,user_id,invoice_id,attempt,method,recipient_email,notes) VALUES (?,?,?,?,?,?,?)`).run(id, data.userId, data.invoiceId, data.attempt||1, data.method||'pdf', data.recipientEmail||null, data.notes||null);
        return this.db.prepare('SELECT * FROM relances WHERE id=?').get(id);
    }
    getRelancesByInvoice(invoiceId) {
        return this.db.prepare('SELECT * FROM relances WHERE invoice_id=? ORDER BY sent_at ASC').all(invoiceId).map(r => this.formatRelance(r));
    }
    ...
    formatRelance(r) {
        return { id: r.id, userId: r.user_id, invoiceId: r.invoice_id, attempt: r.attempt, method: r.method, recipientEmail: r.recipient_email, sentAt: r.sent_at, notes: r.notes };
    }
    ...
    saveExchangeRate(data) {
        ...
        if (existing) {
            ...
            this.db.prepare('INSERT INTO exchange_rates (id,user_id,currency,rate) VALUES (?,?,?,?)').run(uuidv4(), data.userId, data.currency, data.rate);
        }
    ...
    saveRecurringInvoice(r) {
        ...
        const items = (r.itemsTemplate || []).map((line, i) => {
            ...
            return { description: parts[0] || '', quantity: parseFloat(parts[1]) || 1, price: parseFloat(parts[2]) || 0, tva: parseInt(parts[3]) || 19 };
        });
    ...
    getDueRecurringInvoices() {
        return this.db.prepare("SELECT * FROM recurring_invoices WHERE active=1 AND next_run IS NOT NULL AND next_run <= date('now')").all();
    }
    ...
    deleteTemplate(id) { this.db.prepare('DELETE FROM document_templates WHERE id=?').run(id); }
    ...
    openSession(userId, openingBalance) {
        ...
        const id = uuidv4();
        const now = new Date().toISOString();
        this.db.prepare('INSERT INTO pos_sessions (id,user_id,opened_at,opening_balance,status) VALUES (?,?,?,?,?)').run(id, userId, now, openingBalance || 0, 'open');
    ...
    getTodayPosSales(userId) {
        const today = new Date().toISOString().split('T')[0];
        return this.db.prepare("SELECT * FROM documents WHERE user_id=? AND is_pos=1 AND date(date) = date(?) ORDER BY created_at DESC").all(userId, today);
    }
